...
void Time::TotalSeconds() // defining a function which calculates + prints the
                          // total number of seconds in the set time
{
  cout << "The total seconds are " << ((60 * minutes) + seconds) << " seconds."
       << endl;
}

void Time::observeTime() // defining a function which prints out the time in
                         // minutes and seconds
{
  while (seconds >= 60) {
    minutes += (seconds / 60);
...
void Time::equalTo(
    Time otherTime) // defining a function which checks if one time object is
                    // equal to another time object
{
...
void Time::lessThan(Time otherTime) // defining a function which checks whether
                                    // one time object is less than another
{
...
void Time::greaterThan(
    Time otherTime) // defining a function which checks whether one time object
...
void Time::subtractTime(Time time1, Time time2) {
  ...
  int totalDiffInSec; // represents the difference in the time objects but
                      // converted to seconds
  int totalSec1; // the total seconds of time1
  int totalSec2; // the total seconds of time 2

  totalSec1 =
      (time1.minutes) * 60 + time1.seconds; // computing total seconds of time1
  totalSec2 = (time2.minutes) * 60 +
              time2.seconds; // computing the total seconds of time2

  if (totalSec1 >= totalSec2) {
    totalDiffInSec = (totalSec1 - totalSec2); // the diff.
    cout << "The time difference is " << totalDiffInSec << " seconds."
         << endl; // printing result
  }

  // repeat of above calculations but with a reversed conditional statment to
  // account for a situation which would otherwise yield a negative number
